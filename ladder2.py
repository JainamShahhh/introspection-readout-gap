...
def run(cfg_dict: dict, tag: str):
    ...
    rows = []
    conds = [(a_, al) for a_ in ("concept", "random", "shuffled") for al in cfg.alphas]
    ...
    with open(f"/out/ladder2_{tag}.json", "w") as f:
        json.dump(out, f)
    results.commit()
