...
def run(cfg_dict: dict, tag: str):
    ...
    with open(f"/out/episodes3_{tag}.json", "w") as f:
    ...
    m.save_pretrained(f"/out/adapter_{tag}")
    import torch as _t
    _t.save({c: v.float().cpu() for c, v in vecs.items()}, f"/out/vectors_{tag}.pt")
    ...
    with open(f"/out/readout3_{tag}.json", "w") as f:
