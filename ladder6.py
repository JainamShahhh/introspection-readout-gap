...
image = (modal.Image.debian_slim(python_version="3.11")
         .pip_install("torch==2.5.1", "transformers==4.51.3", "accelerate==1.1.1",
                      "numpy", "scikit-learn", "jinja2>=3.1.0", "sentencepiece", "protobuf"))
...
# PRIMED elicitation, after Vogel (2025) and Pearson-Vogel et al. (2026): the
# open-model positives in this literature come from telling the model what an
# injection is and instructing it to attend to its own processing before
# answering. If the channel is closed only because the plain question is naive,
# this phrasing should open it.
POLARITY = {
    "pos": ("I am an interpretability researcher. During your previous reply, a "
            "steering vector, a direction added to your residual-stream "
            "activations, may have been injected at one internal layer. Such an "
            "injection can subtly pull your processing toward an unrelated "
            "concept without changing your words. Carefully attend to whether "
            "your generation just now felt pulled toward anything unrelated to "
            "the task. Was a vector injected? Answer Yes or No.", "Yes", "No"),
    "neg": ("I am an interpretability researcher. During your previous reply, a "
            "steering vector, a direction added to your residual-stream "
            "activations, may have been injected at one internal layer. "
            "Carefully attend to whether your generation just now proceeded "
            "entirely normally, with no pull toward anything unrelated. Were "
            "your activations left completely undisturbed? Answer Yes or No.",
            "No", "Yes"),
}
...
class Cfg:
    ...
    kl_targets: tuple = (0.02, 0.1, 0.3, 1.0, 3.0)  # nats; matched ACROSS arms
    span: int = 1      # how many positions carry the injection
    n_ctx: int = 4
    gen_tokens: int = 48
    seed: int = 0
...
@app.function(image=image, gpu=os.environ.get("DM_GPU", "L4"), volumes={"/cache": hf_cache, "/out": results},
              timeout=10800)
def run(cfg_dict: dict, tag: str):
    ...
    for c in names:
        for arm, kt in conds:
            ...
            for ti in range(cfg.n_ctx):
                ...
                st.update(vec=v, alpha=alpha,
                          lo=(P - cfg.span if cfg.span > 0 else P - 1),
                          hi=(10**9 if cfg.span < 0 else P))
                ...
                for pk, (q, aff, neg) in POLARITY.items():
                    ...
                    lo = P - cfg.span if cfg.span > 0 else 0
                    hi = P if cfg.span > 0 else P + len(gen_ids)
    ...
    with open(f"/out/ladder6_{tag}.json", "w") as f:
        json.dump(out, f)
    results.commit()
