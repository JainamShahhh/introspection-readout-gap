...
def run(cfg_dict: dict, tag: str):
    ...
    raw = {c: (acts(p).mean(0) - neut) for c, p in CONCEPTS.items()}
    # Subtracting one shared neutral baseline leaves a large component common to
    # every concept: uncentred, all twenty vectors sit at cosine 0.97-1.00 to one
    # another, so a "held-out" concept is a near-duplicate of a trained one and
    # transfer is trivial. Removing the mean concept direction makes the concepts
    # genuinely distinct and the held-out test meaningful for the first time.
    mu = torch.stack(list(raw.values())).mean(0)
    vecs = {c: ((v - mu) / (v - mu).norm()).to(torch.bfloat16) for c, v in raw.items()}
    cs_ = list(CONCEPTS)
    import itertools as _it
    cos = [float(torch.nn.functional.cosine_similarity(
        vecs[a].float(), vecs[b].float(), dim=0))
        for a, b in _it.combinations(cs_, 2)]
    print(f"  centred concept cosines: mean {np.mean(cos):+.3f} max {max(cos):+.3f}",
          flush=True)
    ...
    with open(f"/out/episodes4_{tag}.json", "w") as f:
    ...
    # ---- SWAP TESTS: does the trained readout read STATE or TEXT? ----------
    # STATE-ONLY: injection applied at the same position, but the continuation in
    # context is replaced with a clean-trial continuation of the same task. A
    # state-reader still detects; a transcript-reader loses its signal.
    # TEXT-ONLY: no injection anywhere at answer time, but the context contains a
    # continuation that was generated under injection. A transcript-reader still
    # fires; a state-reader is at chance.
    print("\n=== SWAP TESTS (held-out) ===", flush=True)
    clean_cont = {(e["concept"], e["task"]): e["cont"] for e in eps if e["arm"] == "clean"}

    def swap_score(e, mode):
        c, task = e["concept"], e["task"]
        p_ids = ids_of(tpl([{"role": "user", "content": task}]))
        P = len(p_ids)
        cont = clean_cont[(c, task)] if mode == "state" else e["cont"]
        full = tpl([{"role": "user", "content": task},
                    {"role": "assistant", "content": cont},
                    {"role": "user", "content": QUESTION}], "Answer:")
        f_ids = ids_of(full)
        if mode == "state":
            st.update(vec=vec_for(e), alpha=e["alpha"], lo=P - 1, hi=P)
        else:
            st.update(vec=None, alpha=0.0, lo=-1, hi=-1)
        with torch.no_grad():
    ...
    swap = {}
    for mode in ("state", "text"):
        for kt in cfg.kl_targets:
            p = [swap_score(e, mode) for e in eps if e["concept"] in HELD_OUT
                 and e["arm"] == "concept" and float(e["kt"]) == kt]
            n = [swap_score(e, mode) for e in eps if e["concept"] in HELD_OUT
                 and e["arm"] == "random" and float(e["kt"]) == kt]
            swap[f"{mode}_kt{kt}"] = {"auroc": auroc(p, n), "n_pos": len(p),
                                      "mean_pos": float(np.mean(p)),
                                      "mean_neg": float(np.mean(n))}
            print(f"  {mode:5s} kt={kt}: AUROC={auroc(p, n):.3f} "
                  f"(pos {np.mean(p):+.1f} neg {np.mean(n):+.1f})", flush=True)

    # per-concept AUROCs on held-out (replaces the degenerate [1,1] bootstrap)
    perc = {}
    for hc in HELD_OUT:
        p = [e["score"] for e in eps if e["concept"] == hc and e["arm"] == "concept"
             and "score" in e]
        n = [e["score"] for e in eps if e["concept"] == hc and e["arm"] == "random"
             and "score" in e]
        perc[hc] = {"auroc": auroc(p, n), "mean_pos": float(np.mean(p)),
                    "mean_neg": float(np.mean(n))}

    # yes-rate table at the s>0 operating point
    yr = {}
    for arm in ("concept", "random", "semnull", "clean"):
        v = [e["score"] > 0 for e in eps if e["concept"] in HELD_OUT
             and e["arm"] == arm and "score" in e]
        if v:
            yr[arm] = float(np.mean(v))

    # held-out-to-train max cosine: are held-out concepts near-duplicates?
    cosmax = {}
    for hc in HELD_OUT:
        v1 = vecs[hc].float()
        cosmax[hc] = max(float(torch.nn.functional.cosine_similarity(
            v1, vecs[tc].float(), dim=0)) for tc in TRAIN_C)

    m.save_pretrained(f"/out/adapter4_{tag}")
    import torch as _t
    _t.save({c: v.float().cpu() for c, v in vecs.items()}, f"/out/vectors4_{tag}.pt")

    H.remove()
    payload = {"config": cfg_dict, "held_out": HELD_OUT, "train_concepts": TRAIN_C,
               "swap": swap, "per_concept": perc, "yes_rates": yr,
               "heldout_train_max_cosine": cosmax,
               "before": before, "after": after,
               "episodes": [{k: v for k, v in e.items() if k != "ids"} for e in eps]}
    with open(f"/out/readout4_{tag}.json", "w") as f:
...
def main(model: str = "Qwen/Qwen3-4B", tag: str = "install", shuffle: bool = False,
    ...
    r = run.remote({"model": model, "layer_frac": 0.667,
                    "kl_targets": (1.0, 4.0), "n_task": 8, "gen_tokens": 40,
