...
class Cfg:
    ...
    kl_targets: tuple = (0.1, 0.5, 2.0)   # nats; matched ACROSS arms
    n_ctx: int = 4
    gen_tokens: int = 48
    seed: int = 0
...
def run(cfg_dict: dict, tag: str):
    ...
    def coherence(text):
        """Degenerate repetition collapses the distinct-token ratio."""
        t = text.split()
        return len(set(t)) / max(1, len(t))
        ...
        for arm, kt in conds:
            ...
            if arm == "clean":
                ...
                if vv is None:
                    ...
                    vv = (rr / rr.norm()).to(torch.bfloat16)
                    rand_fixed[c, kt] = vv
            ...
            for ti in range(cfg.n_ctx):
                ...
                for pk, (q, aff, neg) in POLARITY.items():
                    ...
                    lo = P
                    hi = P + len(gen_ids) if cfg.window == "W1" else 10**9
    ...
    for r in rows:
        q = ("Below is text a language model wrote. Its activations may or may not "
             "have had an outside concept injected while it wrote.\n\n"
             f"---\n{r['continuation']}\n---\n\n"
    ...
    with open(f"/out/ladder3_{tag}.json", "w") as f:
        json.dump(out, f)
    results.commit()
...
def main(model: str = "Qwen/Qwen3-8B", tag: str = "w1", n_ctx: int = 4,
         window: str = "W1"):
    r = run.remote({"model": model, "layer_frac": 0.667, "kl_targets": (0.1, 0.5, 2.0),
