...
def run(cfg_dict: dict, tag: str):
    ...
    def make(concept, arm, kt, task):
        ...
        P = len(p_ids)
        v = None if arm == "clean" else (vecs[concept] if arm == "concept"
                                         else rand_dir[concept])
    ...
    def vec_for(e):
        c, arm = e["vec_key"]
        return None if arm == "clean" else (vecs[c] if arm == "concept" else rand_dir[c])
    ...
    with open(f"/out/episodes_{tag}.json", "w") as f:
    ...
    tr = [e for e in eps if e["concept"] in TRAIN_C]
    ...
    with open(f"/out/readout_{tag}.json", "w") as f:
